...
********************Creating map frpm Virus levells on 4rth wek of July 2021 **************************
 Data1 = read.table("/home/sbaby/Desktop/Latest_data/Map/MapVirus.csv", header =TRUE, sep = ",")
 Data1 = as.data.frame(Data1)                          
 pal <- colorQuantile(palette = "Blues",domain = myfile@data$area) 
 pal1 <- colorNumeric(palette = c("yellow","dark orange","blue"),domain = c(0,2.28,120)) 
 Data1[Data1$Site %in% c("Allanfearn","Helensburgh","Carbarns","Hamilton","Philipshill","Seafield","East Calder","Linlithgow","Shieldhall","Dalmuir","Paisley","Daldowie","Nigg","Peterhead","Kirkwall","Lerwick","Hatton","Forfar","Stirling","Falkirk","Alloa","Meadowhead","Stevenston","Linlithgow","Dunfermline","Levenmouth","Kirkcaldy","Troqueer","Lockerbie","Galashiels","Hawick","Stornoway", "Kirkcaldy"),] -> Virus2                       
 Map_Population2<- map %>% addTiles() %>% setView(-3.475300, 55.89687, zoom = 10) %>% addPolygons(fillColor = ~pal(area),fillOpacity = .8,color = "#C0C0C0",weight = 1,label = myfile@data$name,labelOptions = labelOptions(noHide = F, textOnly = TRUE, style= list("font-family" = "Helvetica")))%>% addLegend(data= Virus2,"bottomright",pal = pal1,values = Virus2$Virus.levels,title = "Virus levels", opacity = 1,  labFormat = function(type, cuts, p) {paste0(labels)})%>% addCircles(lng = Virus2$lon,lat = Virus2$lat, label = Virus2$Site,weight =20, color = ~pal1(Virus2$Virus.levels),opacity = 1)%>% addLabelOnlyMarkers(lng = Selected_Sites$lon,lat = Selected_Sites$lat, label = Selected_Sites$Site,labelOptions = labelOptions(noHide = T, textOnly = T,opacity = 1,textsize = "19px", style = list("font-weight" = "bold","font-family" = "Helvetica",opacity = 1)))                         
 saveWidget(widget = Map_Population1, file = "/home/sbaby/Desktop/Map_Population_7.html")
