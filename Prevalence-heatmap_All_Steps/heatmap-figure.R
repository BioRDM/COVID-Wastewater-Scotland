...
allData = as.data.frame(read.table("RNAMonitoring_File_Feb_2022.csv", header =TRUE, sep = ","))

allPositive <- filter(allData, allData$N1.Description == "Positive")$Million.Gene.Copies.Per.Person.per.Day
nonPositive <- c( filter(allData, allData$N1.Description == "Weak Positive")$Million.Gene.Copies.Per.Person.per.Day,
                  filter(allData, allData$N1.Description == "Positive (DNQ)")$Million.Gene.Copies.Per.Person.per.Day,
                  filter(allData, allData$N1.Description == "Negative")$Million.Gene.Copies.Per.Person.per.Day )

pos_start = round(quantile(nonPositive, 0.75, na.rm=TRUE), digits = 1)
pos_end = round(quantile(allPositive, 0.75, na.rm=TRUE), digits = 1)

# pos_start 2.9, pos_end = 88
...
DF_new = Data2[row.names(Data2) %in% c("Allanfearn","Helensburgh","Carbarns","Hamilton","Philipshill","Seafield","East Calder","Linlithgow","Shieldhall","Dalmuir","Paisley","Daldowie","Nigg","Peterhead","Kirkwall","Lerwick","Hatton","Forfar","Stirling","Falkirk","Alloa","Meadowhead","Stevenston","Linlithgow","Dunfermline","Levenmouth","Kirkcaldy","Troqueer","Lockerbie","Galashiels","Hawick","Stornoway"),32:100] 
...
#fName = paste("figure_prevalence_heatmap",pos_start,".jpeg", sep = "")
